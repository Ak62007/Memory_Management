...
snek_object_t *snek_add(snek_object_t *a, snek_object_t *b) {
  ...
  if (a->kind == INTEGER){
    ...
    if (b->kind != STRING){
      ...
      int new_str_len = strlen(a->data.v_string) + strlen(b->data.v_string) + 1;
      char *temp_str = calloc(new_str_len, sizeof(char));
      strcat(temp_str, a->data.v_string);
      strcat(temp_str, b->data.v_string);
      snek_object_t *new_string = new_snek_string(temp_str);
      free(temp_str);
      return new_string;
    }
  }
    ...
    if (b->kind != ARRAY){
      ...
      for (int i=0;i<a->data.v_array.size;i++){
        snek_array_set(new_arr, i, snek_array_get(a, i));
      }

      for (int j=0; j<b->data.v_array.size;j++){
        snek_array_set(new_arr, a->data.v_array.size + j, snek_array_get(b, j));
      }
      return new_arr;
    }
  }
